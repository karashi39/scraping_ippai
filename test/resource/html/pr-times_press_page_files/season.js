 ...
 *
 */
var season_words  = ['今日はエイプリルフールです','今日は、エイプリルフールです'];
var flg_done      = false;


function season(word,page){

    /*
     タイマー
     ----------------------------------------------------------------------*/
    var fromDate = moment();
    var toDate   = moment('2016-02-15 00:00');
    if(toDate.diff(fromDate) <= 0) return false;


    /*
     PC 検索ワード内にクエリにあるか確認
     ----------------------------------------------------------------------*/
    var isSearchSeason  = _cheackSeasonWordFromMyQuery(season_words);


    /*
     シーズンイベント判定
     ----------------------------------------------------------------------*/
    var isSeason = ($('main').attr('data-season') === 'true' ) ? true : false;
    var isSp     = (navigator.userAgent.indexOf('iPhone') > 0 || navigator.userAgent.indexOf('iPod') > 0 || navigator.userAgent.indexOf('Android') > 0) ? true : false;


    if(flg_done) {
        isSeason       = false;
        isSearchSeason = false;
    }
    ...
    if(typeof word != 'undefined'){
        ...
        var search_word = $("aside.search form input[type=search]").val();
        var wordArray = search_word.replace(/　/g," ").split(" ");
        var isSearchSeason = false;

        $.each(season_words, function(i,val) {
            if($.inArray(val, wordArray) !== -1){
                isSearchSeason = true;
            }
        });


        if(isSearchSeason && page == 1){
            //return
        }else if(!isSearchSeason){
            $('.js-season-balloon').css('display','none');
            return false;
        }else{
            $('.js-season-balloon').css('display','block');
            return false;
        }
    }
    ...
    if(isSeason || isSearchSeason){

        //clear
        $('.js-season-balloon').remove();


        $('body').addClass('season-valentine');
        $('.js-season-insert').addClass('balloon_parent').prepend('<div class="season-balloon js-season-balloon"></div>');



        var $balloon = $('.js-season-balloon');
        ...
        $balloon.ready(function(){
            ...
            //出現
            if(!isSp){
                $balloon.css({'top':$(window).height() / 2 , 'margin-top': $(window).scrollTop() - 150 })
                    .delay(700).animate({'margin-left':'732px'}, 600,function(){
                    $(this).addClass('sway');
                });
            }else{
                $balloon
                    .delay(400).animate({'right':'-49px'}, 600,function(){
                    $(this).addClass('sway');
                });
            }


            $('.js-season-insert').on('click','.js-season-balloon',function(){
                flg_done = true;

                if(isSp){
                    $balloon
                        .removeClass('sway').addClass('x_sway')
                        .animate({'top':'-200px' ,'right':'200px','margin-left':'600px'}, 4000);

                }else{
                    $balloon.removeClass('sway').addClass('x_sway')
                        .animate({'top':'-241px' ,'margin-left':'500px'}, 6300);
                    setTimeout(function(){
                        $('.balloon_parent').css('overflow','visible');
                    }, 1000);

                    //TweenMax.to('.js-season-balloon', 5, {
                    //    y        :'-1000px',
                    //    x        :'-100px',
                    //    rotation : 0,
                    //});
        ...
        $(window).scroll(function(){
            var now  = $(window).scrollTop() +  (windowHeight / 2);

            if(isSp){
                if(!$('html').hasClass('sb-active')){
                    if(limit_line != 'unlimited' && now > limit_line || flg_done){
                        $balloon.fadeOut();
                    }else{
                        $balloon.fadeIn();
                    }
                ...
                $balloon.css("margin-top", ( $(window).scrollTop()) - 150);
                if(limit_line != 'unlimited' && now > limit_line || flg_done){
                    $balloon.fadeOut();
                }else{
                    $balloon.fadeIn();
        ...
        if(!isSp){
            ...
            $(window).resize(function() {
                ...
                timer = setTimeout(function() {
                    $balloon.css('top',$(window).height() / 2);
...
$(".button-search").on('click', function() {
   ...
   $('.js-season-balloon').hide();
    ...
    $.each(season_words, function(i,val) {
        if($.inArray(val, wordArray) !== -1 && !flg_done){
            isSearchSeason = true;
        }
    });

    if(isSearchSeason){
        season(search_word,1);
    }
...
/*
 SP版 検索画面 前に戻るボタン
 ----------------------------------------------------------------------*/
$(".button-back-main-menu").on('click', function() {

    $('.js-season-balloon').hide();

    //deta属性から取得するとストリングの'true'
    if($('main').attr('data-season') == 'true'){
        $('.js-season-balloon').show(1000);
    }
 ...
 *
 ----------------------------------------------------------------------*/
var _cheackSeasonWordFromMyQuery = function(season_words){
    var res            = false;
    var url            = location.href;
    var tmp_params     = url.indexOf('?') != -1 ? url.split("?") : [];
    var params         = url.indexOf('&') != -1 ? tmp_params[1].split("&") : [];
    var paramArray     = [];
    ...
    if(typeof paramArray['search_word'] != 'undefined'){
        var tmp_search_word = decodeURIComponent(paramArray['search_word'].replace(/\+/g, '%20'));
        var search_word     = tmp_search_word.replace(/　/g," ").split(" ");
        $.each(search_word, function(i,val) {
            if($.inArray(val.toLowerCase(), season_words) !== -1){
                res = true;
            }
        });
    }
...
/*
 limit_line
 ----------------------------------------------------------------------*/
var _setLimitLine = function(){
    var res = (typeof $('.js-season_effect_end').offset() != 'undefined' ) ? $('.js-season_effect_end').offset().top : 'unlimited';
    return res;
}
